...
export class PoiService {
  ...
  async siteConditions(point: LatLng): Promise<SiteLookup> {
    ...
    if (snapshot !== null) {
      return {
        site: siteConditions(snapshot.elements, point),
        access: accessContext(snapshot.elements),
        available: true,
      };
    }
    ...
    if (!lookup.available) return { site: {}, access: { barriers: [], passages: [] }, available: false };
    const exact = lookup.elements.filter((element) => matchesSiteQuery(element, point));
    return { site: siteConditions(exact, point), access: accessContext(exact), available: true };
  }
  ...
  private async loadFacilities(cell: string): Promise<OpenStreetMapSnapshot> {
    ...
    if (cached !== null && this.isFresh(Date.parse(cached.fetchedAt))) {
      return {
        ...
        cacheHit: true,
        stale: false,
        via: cached.source ?? 'overpass',
      };
    }

    const source: 'geoapify' | 'overpass' = this.geoapify.enabled ? 'geoapify' : 'overpass';
    ...
    try {
      ...
      this.logger.log(`${label} POI query for ${cell}: ${facilities.length} facilities in ${Date.now() - started} ms`);
      await this.cache.set(entry);
      return { facilities: entry.facilities, fetchedAt: entry.fetchedAt, cacheHit: false, stale: false, via: source };
    } catch (error) {
      if (cached !== null) {
